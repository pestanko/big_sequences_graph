/**
 * Created by Peter Stanko on 9/14/15.
 */
...
function Application()
{
    ...
    {
        this.wss = new WebServer();
        console.log("[INFO] Listening on port: " + port);
        //Create a server
        var server = http.createServer(this.wss.handleRequest);

        //Lets start our server
        server.listen(port, function(){
            //Callback triggered when server is successfully listening. Hurray!
            console.log(" [INFO] Server listening on: http://localhost:%s", port);
        });

        this.wss.call_config = function (response) {
            response.end(JSON.stringify(_this.getCfgMsg()));
        };

        this.wss.call_get = function(response, msg)
        {
            response.end(JSON.stringify(_this.handleTileMsg(msg)));
        };

        this.wss.call_interval = function(response, msg)
        {
            var level = msg.level;
            var beg = msg.beg;
            var end = msg.end;
            ...
            {
                ret_data.push(_this.holder.getTileData(level, i));
            }
            var resp = {
                type: "tiles",
                level: level,
                ...
                raw: _this.holder.levels[level].raw,
                data: ret_data
            };
